...
**/
	if ((*RootPtr) == NULL)
	{
		tBSTNodePtr new_element = malloc(sizeof(struct tBSTNode));
		if (new_element == NULL)
		{
			exit(1);
		}
		else
		{
			...
			new_element->BSTNodeCont = Content;
			new_element->Key = K;
			(*RootPtr) = new_element;
		}	
	}
...
**/
	...
	else if ((*RootPtr)->Key == K)
	{
		...
		{
			tBSTNodePtr join_right = (*RootPtr);
			(*RootPtr) = join_right->RPtr;
			free(join_right);
		}
		//nemá pravý podstrom
		else if ((*RootPtr)->RPtr == NULL)
		{
			tBSTNodePtr join_left = (*RootPtr);
			(*RootPtr) = join_left->LPtr;
			free(join_left);
		}
		//má oba stromy
		else
