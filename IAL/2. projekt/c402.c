...
**/
	...
	if ((*RootPtr) == NULL)
	{
		tBTNodePtr new_element = malloc(sizeof(struct tBTNode));
	...
	else
	{
		...
		tBTNodePtr second_helper;
		while (helper != NULL)
		{
			if (Content < helper->Cont)
			{
				second_helper = helper;
				helper = helper->LPtr;
		    }
			else if (Content > helper->Cont)
			{
				second_helper = helper;
				helper = helper->RPtr;
		    }
		...
		else
		{
			...
			if (Content < (second_helper)->Cont)
			{
				second_helper->LPtr = new_element;
			}
			else
			{
				second_helper->RPtr = new_element;
			}
		}
	}
...
**/
	...
	else
	{
		...
		Leftmost_Postorder(RootPtr, &stack, &b_stack);
		
		while (!SEmptyP(&stack))
		{
			RootPtr = STopPopP(&stack);
			SPushP(&stack, RootPtr);
			zleva = STopPopB(&b_stack);
			if (zleva)
			{
				SPushB(&b_stack, false);
				Leftmost_Postorder(RootPtr->RPtr, &stack, &b_stack);
			}
			else
			{
				STopPopP(&stack);
